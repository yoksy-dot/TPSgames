...
public class NETSideAttackSystem : NetworkBehaviour
{
    public GameObject ArrowPrehab;
    GameObject ChildShoot;
    NETShootBulletAuto Auto;
    ...
    {
        ;
        Auto = GetComponent<NETShootBulletAuto>();
        time = time2 = 0.0f;
        Did = false;
        Shooted = 0;
    }
    ...
    void FixedUpdate()
    {
        if (fire)
        {
            Auto.CmdShotOnly(ChildShoot, ArrowPrehab, ShootSpeed, NUM, Lv);
            fire = false; ;
        }
    }
}
